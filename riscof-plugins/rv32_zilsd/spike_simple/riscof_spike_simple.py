...
class spike_simple(pluginTemplate):
    ...
    #       self.isa += 'd'
      if "C" in ispec["ISA"]:
          self.isa += 'c'
      self.isa += 'zicsr_zilsd_zca_zclsd'
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' -m8796093022208 --misaligned --isa={0} +signature={1} +signature-granularity=4 {2}'.format(self.isa, sig_file, elf)
            simcmd = simcmd + ';' + self.dut_exe + ' -m8796093022208 --misaligned --isa={0} --log-commits -l my.elf 2> {1}'.format(self.isa, log_file)
          else:
            simcmd = 'echo "NO RUN"'
